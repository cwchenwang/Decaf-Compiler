...
public class Parser extends Table {
    ...
    private SemValue parse(int symbol, Set<Integer> follow) {
        ...
        //获得begin和end集合
        Set<Integer> mBeginSet = beginSet(symbol);
        Set<Integer> mEndSet = followSet(symbol);
        mEndSet.addAll(follow);
        follow = mEndSet;

        boolean aheadInBegin = mBeginSet.contains(new Integer(lookahead));
        if(!aheadInBegin) { //出错处理
            error();
            Integer mLookahead = new Integer(lookahead);
            while(!mBeginSet.contains(mLookahead) && !mEndSet.contains(mLookahead)) {
                lookahead = lex();
                mLookahead = new Integer(lookahead);
            } 
            if(mBeginSet.contains(mLookahead)) {
                return parse(symbol, follow);
            } else if(mEndSet.contains(mLookahead)) {
                return null;
            }
        }
        ...
        for (int i = 0; i < length; i++) {
            if (params[i + 1] == null) return null;
        }
