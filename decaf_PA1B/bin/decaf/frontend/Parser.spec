...
'+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}' ':'
SEALED ARRAYCONCAT SCOPY DOUBLEMO IN VAR GUARD_SEP DEFAULT FOREACH
LCOMP RCOMP
%%
...
ForeachStmt     :   FOREACH '(' BoundedVariable IN Expr WhileHelper ')' Stmt
                    {
                        $$.stmt = new Tree.Foreach($3.stmt, $5.expr, $6.expr, $8.stmt, $1.loc);
                    }
                ;

WhileHelper     :   WHILE Expr
                    {
                        $$.expr = $2.expr;
                    }
                |   /* empty */
...
Expr4S          :   Expr4SX ExprT4S
                    {
                        $$.expr = $1.expr;
                        if ($2.svec != null) {
                            for (int i = $2.svec.size() - 1; i >= 0; --i) {
                                if($2.svec.size() == 1) {
                                    $$.expr = new Tree.ArrayConcat($1.expr, $2.evec.get(0), $2.lvec.get(i));
                                } else {
                                    if (i == 0) {
                                        $$.expr = new Tree.ArrayConcat($1.expr, $$.expr, $2.lvec.get(i));
                                    } else {
                                        if(i == $2.svec.size() - 1) $$.expr = $2.evec.get(i);
                                        $$.expr = new Tree.ArrayConcat($2.evec.get(i-1), $$.expr, $2.lvec.get(i));
                                    }
                                }
                            }
                        }
...
ExprT4S         :   Oper4S Expr4SX ExprT4S
                    {
                        $$.svec = new Vector<Integer>();
                        $$.lvec = new Vector<Location>();
                        $$.evec = new Vector<Expr>();
...
ExprT4SX        :   Oper4SX Expr5 ExprT4SX
                    {
                        $$.svec = new Vector<Integer>();
                        $$.lvec = new Vector<Location>();
                        $$.evec = new Vector<Expr>();
                        ...
                        if ($2.vec != null) {
                            for (SemValue v : $2.vec) {
                                if (v.expr != null && v.expr1 != null) {
                                    $$.expr = new Tree.Range($$.expr, v.expr, v.expr1, $1.loc);
                                } else if (v.expr != null && v.expr2 != null) {
                                    $$.expr = new Tree.Default($$.expr, v.expr, v.expr2, $1.loc);
                                } else if (v.expr != null) {
...
ExprT8          :   '[' Expr RExprHelper ExprT8
                    {
                        SemValue sem = new SemValue();
                        sem.expr = $2.expr;
                        if($3.expr1 != null) {
                            sem.expr1 = $3.expr1;
                        } else if($3.expr2 != null) {
                            sem.expr2 = $3.expr2;
                        }
...
Expr9           :   Constant
                ...
                |   LCOMP Expr FOR IDENTIFIER IN Expr CompHelper
                    {
                        $$.expr = new Tree.Comprehension($2.expr, $6.expr, $7.expr, $4.ident, $2.loc);
                    }
                ;

CompHelper      :   IF Expr RCOMP
                    {
                        $$.expr = $2.expr;
                    }
                |   RCOMP
...
ConstantArray   :   Constant ConstHelper
                    {
...
ConstHelper     :   ',' Constant ConstHelper
                    {
                        $$.elist = new ArrayList<Tree.Expr>();
                        $$.elist.add($2.expr);
...
GuardedHelper   :   GuardedStmt '}'
                    {
                        $$.stmt = new Tree.Guarded($1.slist, $1.loc);
                    }
                |   '}'
                    {
...
GuardedStmt     :   IfSubStmt IfSubStmtHelper
                    {
...
IfSubStmtHelper :   GUARD_SEP IfSubStmt IfSubStmtHelper
                    {
                        $$.slist = new ArrayList<Tree>();
                        $$.slist.add($2.stmt);
